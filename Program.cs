...
namespace MobaXtermPasswordRecovery
{
    internal static class Program
    {
        ...
        static int Main(string[] args)
        {
            ...
            // 创建根命令
            var rootCommand = new RootCommand("MobaXterm Password Recovery Application");
        ...
        private static void LoadSubCommands(Command rootCommand, Option<bool> debugOption)
        {
            ...
            // 查找 MobaXtermPasswordRecovery.Softwares 命名空间下的所有类型
            var toolTypes = assembly
                .GetTypes()
                .Where(t => t.Namespace == "MobaXtermPasswordRecovery.Softwares" && t.GetMethod("Run") != null)
                .ToList();
        ...
        private static void CallSpecificClass(Type type, string[] classArgs)
        {
            ...
            catch (Exception e)
            {
                ...
                Logger.Info(
                    $"An error occurred while calling class {type.Name}: {rootCause.GetType().Name}: {rootCause.Message}",
                    label: "[-]"
                );
            }
        }
